...
$pin = new pin();
$sql = "SELECT * FROM hyq_city";
$db_citys = $db_hyq_read -> fetch_result($db_hyq_read ->  query($sql));
foreach($db_citys as $db_city){
	$spell = $pin -> Pinyin($db_city['cname'],'UTF-8');
	$sql1 = "UPDATE hyq_city SET spell = '{$spell}' WHERE id = '{$db_city['id']}'";
	$db_hyq_write ->  query($sql1);	
}
?> 