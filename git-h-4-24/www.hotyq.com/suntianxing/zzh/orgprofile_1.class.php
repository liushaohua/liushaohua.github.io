...
class orgprofile extends dbcache{
	//查询数据的资料
  	function get_orginfo($uid){
		global $db_hotyq_read;	//读取数据
 		$sql = "SELECT  id,uid,create_time,province_id,city_id,district_id,type,state,introduce,production,heeler,honor,business_num,legal_person FROM hyq_orginfo WHERE uid = {$uid}";
  		//$sql = "SELECT  * FROM hyq_uinfo WHERE uid = {$uid}";
		$userlist=$this->get_hyq_sql_array($sql,1);//读取一条数据从缓存
		return $userlist;
		//var_dump($userlist);
  	}

	public function update_org_info($uid,$nickname,$orginfo = array()){
		global $db_hyq_write;
			$ip = getIP();
			//修改orgrinfo表 资料的SQL语句
			$sql =" UPDATE hyq_orginfo SET ";							
			...
			$re = $this->update_org_name($uid,$nickname);//插入机构名字
			if($result && $re){
				return 1000;
			}else{
				return 1112;
	...
	//机构名字输入
	public function update_org_name($uid,$nickname){
		global $db_hyq_write;	
	...
	public function business_num_exist($business_num){
		global $db_hyq_read;
		$sql = "SELECT id FROM hyq_orginfo WHERE business_num='{$business_num}'";
		$query = $db_hyq_read -> query($sql);
		$result = $db_hyq_read -> fetch_array($query);
		if($result){
	...
	//工商号开始绑定
	public function banding_business_num($business_num){
		global $db_hyq_write;
		$sql = "UPDATE hyq_orginfo SET 
	...
	//如果机构是法人择绑定身份证
	//检测身份证格式
	public function check_identity_card($business_num){
		if(empty($business_num)){
			return 1115;//身份证为空！
		}else{
			$pattern="/^(?:\d{15}|\d{18})$/";
	...
	//检测身份证是否存在
	public function identity_card_exist($business_num){
		global $db_hyq_read;
		$sql = "SELECT id FROM hyq_uinfo WHERE business_num='{$business_num}'";
		$query = $db_hyq_read -> query($sql);
		$result = $db_hyq_read -> fetch_array($query);
		if($result){
			return 1000;//身份证已经绑定过！
		}else{
			return 1117;//身份证可以绑定!
		} 
	}
	//开始绑定身份证	
	public function banding_identity_card($uid,$business_num){
		global $db_hyq_write;
		$sql = "UPDATE hyq_uinfo SET 
				identity_card = {$business_num}
				WHERE uid = {$uid}";
		$result = $db_hyq_write->query($sql);
		if($result){
			return 1000;
