...
class redis_class{
	...
	public function hvals($key) {
		$this -> connect();
		return $this->redis->hvals($key);// 全部hash数据
	}
	/**
	 * 获取 Hash 的大小 域的数量
	 * @param string $key hash名称  
	...
	*/
	...
	//add date 2015-1-2 by wangyi
	public function sinter($param){
		$this -> connect();
		return $this->redis->sinter($param);
	}
	public function sinterstore($param){
		$this -> connect();
		return $this->redis->sinterstore($param);
	}
	public function sunion($param){
		$this -> connect();
		return $this->redis->sunion($param);
	}
	public function sunionstore($param){
		$this -> connect();
		return $this->redis->sunionstore($param);
	}
	...
	public function zrange($key, $star, $end, $config=""){
		$this -> connect();
		return $this->redis->zrange($key, $star, $end, $config="");
	}
	public function zrevrange($key, $star, $end, $config=""){
		$this -> connect();
		return $this->redis->zrevrange($key, $star, $end, $config="");
	}
	public function zinterstore($key, $key_arr, $weights=null, $aggregate=null){
		$this -> connect();
		return $this->redis->zinterstore($key, $key_arr);
	}
	public function zunionstore($key, $key_arr, $weights=null, $aggregate=null){
		$this -> connect();
		return $this->redis->zunionstore($key, $key_arr);
	}
