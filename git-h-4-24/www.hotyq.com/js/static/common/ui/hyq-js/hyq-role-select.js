...
$.widget("ui.HYQRoleManager",{
	...
	renderCheck : function (data) {
		var Roles = data.userRoles;
			setTimeout(function () {
				...
				//console.log('sss',$('.rr-children-box').length);
				//console.log('sss',$('.rr-list-inner').html());
				for (var i = 0,len = Roles.length; i < len; i++) {
					for (var j = 0,jLen = BoxLen; j < jLen; j++) {console.log('aa');
						...
						for (var w = 0,wLen = cCheck.length; w < wLen; w++) {
							//console.log('UU',cCheck.eq(w).val(),+Roles[i]);
	...
	renderin:function(userRoles,userCustomRoles){
		...
		try{
			...
			for(var i=0;i<userRoles.length;i++){
				...
				// console.log('最初渲染-model-wangyifan')
			...
			console.log('克隆tags_end')
			console.log(s.tags_end)
			// console.log('通知要渲染-tags-wangyifan')
			// console.log(tags)
			//通知要渲染
			$s.trigger({
	...
	addTagByModel:function(model){
		var s = this,$s = $(this.element);
		var isAdded = false;
		// console.log('根据model增加上面标签-isadded-wangyifan')
		if(s.tags.length>0){
			//判断是否存在  存在就不加了
...
// <span class="selected-role inline-block"><span class="role-value"></span> &nbsp;&nbsp;<a class="close-btn-tiny"></a></span>
	...
	_create:function(){
		...
		$(s.options.manager.element).on($._HYQEVT_.RR.TAGS_REDUCED,function(event){
			...
			//console.log('开删');
			//console.log(event)
			//console.log("SELECTOR = "+".selected-role[_id="+event.reduced.id+"]");
	...
	_create:function(){
		...
		this.model = this.options.model;
			...
			$(s.options.manager.element).on($._HYQEVT_.RR.TAGS_FULL,function(event){
				// console.log("标签已满");
				// console.log(s.options.manager.tags.length)
			...
			$(s.options.manager.element).on($._HYQEVT_.RR.TAGS_RENDERING,function(event){
				...
				// console.log(event)
				for(var i=0;i<event.tags.length;i++){
					//console.log(event.tags[i]+" === "+s.options.model.id);
