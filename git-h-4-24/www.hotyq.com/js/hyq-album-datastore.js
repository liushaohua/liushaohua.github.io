;(function($){
	$.widget("ui.HYQAlbumDatastore",{
			...
			for(var i=0;i<s.options.albumData.length;i++){
				...
				if(s.options.albumData[i]!=null){
					...
					if(s.options.albumData[i].id==id){	
						...
						//position
						//console.log('index',i);
						$item.eq(i).appendTo($cBox);
						s.options.albumData = s.options.albumData.concat(s.options.albumData.splice(i,1));
					}
				}
			}
			var $item2 = $('.album-wrapper .album-item');
			for(var j=0;j<$item2.length;j++){
				$item2.eq(j).attr('itemindex',j);
			}

			var mIndex = 0; 
			for(var m=0;m<s.options.albumData.length;m++){
				if(s.options.albumData[m] ==null){
					mIndex = m+1;
					break;
				}
			}
			for(var cI=mIndex;cI<$item2.length;cI++){ 
				$item2.eq(cI).find('.invite-btn').hide();
				$item2.eq(cI).find('form').hide();
			}
		...
		_setDeletedItems:function(deleteds){/*deleteds 是个图片ID的数组*/
			var s = this,$s = $(this.element);
			//console.log(deleteds,'_setDeletedItems');
		...
		_create:function(){
			...
			$s.on(s._E.deleted_in_server,function(event){
				...
				//console.log("最后的数据");
				//console.log("最后的数据",s.options.albumData);
