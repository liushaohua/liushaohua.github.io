...
$.widget("ui._HYQAlbumUploader",{
		...
		//构造函数
		 _create:function(){
			...
			//this._form = $('<form>').attr({'enctype':'multipart/form-data',"name":formName,'method':'post',"action":uploadApi,"target":frameName}).appendTo($el);
			...
			this.clickMask.click(function(){
				var $this = $(this);
				if ($this.next('.invite-btn').is(':visible')) {
					//$(this).parent().find('input').click();		//lsh 注释
				}
			});
			//当文件域有所改变时，提交上传的隐藏表单
			...
			$s.on(_ALBUM_UPLODER_IMG_READY,function(event){
				...
				$s.hide();
				//new
				s._form.parents('.album-item').next('.album-item').find('.invite-btn').fadeIn(500);
				s._form.parents('.album-item').next('.album-item').find('form').show();
			});
...
$.widget("ui._HYQAlbumItem",{
		...
		_create:function(){//console.log('create------------');
			...
			var uploadApi = this.options.uploadApi||null;
		 	...
		 	s.uploader.on(_ALBUM_UPLODER_UPLOADING,function(event){
	  		...
	  		s.uploader.on(_ALBUM_UPLODER_IMG_READY,function(event){
	  			//console.log("AlbumManager收到某个Uploader上传后传回来的信息");
	  			//console.log(event);
				...
				s.options.albumStore.setItem(item,s.element.attr('itemIndex'),s);
				//console.log('设置',s.options.pos,s.element.attr('itemIndex'));
			...
			$(s.options.albumStore.element).on("hyq/album/data/deleted",function(event){
					//console.log("AlbumItem接收到删除图片的通知");
					//console.log(event)
					s.manager.toggleBtnGroup(true);
					s.busyMsk.mute();
	...
	$.widget("ui.HYQAlbumManager",{
		...
		deleteImages:function(imageIds){
			//imageIds是一个ID数组 只有一张图也要写成数组的形式
			// console.log("需要删除的图片数组");
		...
		_create:function(){
            ...
            $(this.options.albumStore.element).on("hyq/album/data/deleted",function(event){
            	...
            	if(event.code=="1000"){
            		s.showMessage('删除照片成功！');
            		// $(s.messageBox).html("删除照片成功!").fadeIn().delay(3000).fadeOut(function(){
            		// 	//检查是否完全删除
            		// 	console.log("姗姗");
            		// 	console.log(s.options.albumStore.options.albumData);
            			if(s.options.albumStore.isAllNull()){
            				s.firstEditMode();
            			}
            		//});  
