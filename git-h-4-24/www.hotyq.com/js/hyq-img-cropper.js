	...
	_create:function(){
		...
		s._file = $('<input>').attr({'type':'file','name':s.options.fileField}).appendTo(s._form);
		 s._file.on('change',function(){
		 	...
		 	//$('.hyq-image-cropper').removeClass('hyq-image-cropper-top');
		 	...
		 	s._form.submit();	
		 	s._file.addClass('hyq-replace');	//lsh新增
		 	$s.parents('.hyq-model-body-de').addClass('hyq-model-body-active');
		 ...
		 s._ifrm.load(function(event){
		 	...
		 	if($.trim(back)=="") return;
		 	s.options.cropper._imageUploaded(back,s._file);
		 });
...
$.widget("ui.HYQImageCropper",{
	...
	_scaleImage:function(node,scaling,zoom,s){
		...
		var newW = Math.round(s._niceSize.w*(scaling-0.5)*zoom);					//console.log(s._niceSize.w,scaling,zoom,'5i5i5i',111111);
		var newH = Math.round(s._niceSize.h*(scaling-0.5)*zoom);
		var dW,dH;
	...
	__handleBackData:function(data,obj){
		//console.log(data);
		...
		if(code!="200"){
			...
			if (!$('[img-mask]').next().length) {
				obj.removeClass('hyq-replace');	//lsh新增
				$(s.element).parents('.hyq-model-body-de').removeClass('hyq-model-body-active');
			}
			//add 不等于200就隐藏
			s.busyMask.hide();
		}
				switch(code){
					...
					case '412':
						...
						return;
					case '415':
						s.$s.trigger({type:'hyq/imgcropper/error-message',hyqMessage:"您只能上传12张照片"});
						return;	
					default:
	...
	_imageUploaded:function(data,obj){//图片上传完毕
		this.__handleBackData(data,obj);
	},
	...
	_imageReady:function(url){//图片准备就绪
		...
		.one('load',function(){
			...
			s._niceSize.w = s.imgDock.width();				//console.log(111111111,'7',s._niceSize.w);
	...
	_saveAction:function(){
		...
		if(avt&&avt2){
			...
			data.zoom = 900/280*100*rate*avt.width()/s._rawSize.w/100;
			...
			if(s.options.type == 2){
				...
				var sW = 
				data.zoom = s.cropper.find("img").width()/s._rawSize.w*(s.options.width/(s.cropper.width()-50));
				//console.debug(data.zoom);
				//图片压缩后宽度/图片原图*(上传后1040/(截图框宽度-旁白))
				
				data.x1 = 0-(s.cropper.find("[img-dock]").position().left-25);
				data.y1 = 0-(s.cropper.find("[img-dock]").position().top-25);
				//移动后的图片-旁白
				...
				var cScleX = -50,
					cScleY = -50;
				if (s._rawSize.h > 3900 && s._rawSize.w > 1800) {
					//cScleX = -20,
					//cScleY = 800;
				}	
				data.x1 = data.x1*s.options.width/(s.cropper.width()+cScleX);
				data.y1 = data.y1*s.options.height/(s.cropper.height()+cScleY);
			}
	...
	//按钮
		...
		$s.on(s._E.uimuted,function(){
 		...
 		$s.find(".zout").click(function(event){
			...
			var zoom = Math.round(150*100/280)/100;
			s._scaleImage(s.imgDock,ratio,1,s);		//console.log('1111111',ratio,'511pp');
